...
export class OpenRange {
    ...
    public static reduceRanges(ranges: OpenRange[], overlap_threshold: number = 1): OpenRange[] {
        if (ranges.length === 0) {
            return [];
        }
        ...
        for (let i = 1; i < sorted_ranges.length; i++) {
            ...
            if (current_range.end_inclusive + overlap_threshold >= next_range.start_inclusive) {
